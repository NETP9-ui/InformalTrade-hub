        ...
        <?php if ($conversations): ?>
            <div class="list-group">
                <?php foreach ($conversations as $conversation): ?>
                    <a href="messages.php?receiver_id=<?= $conversation['user_id'] ?>" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
                        <span><?= htmlspecialchars($conversation['username']) ?></span>
                        <span>
                            <?php if ((int) $conversation['unread_count'] > 0): ?>
                                <span class="badge bg-primary rounded-pill me-2"><?= (int) $conversation['unread_count'] ?></span>
                            <?php endif; ?>
...
$stmtRead = $pdo->prepare("UPDATE messages SET is_read = 1 WHERE sender_id = ? AND receiver_id = ? AND is_read = 0");
$stmtRead->execute([$chat_with, $current_user]);
