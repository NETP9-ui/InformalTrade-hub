...
// Load marketplace categories once for the create and edit product forms
$categories = $pdo->query("SELECT * FROM categories")->fetchAll();

function uploadProductImage($fileKey, &$error) {
    // Image upload is optional; return an empty value when no file was submitted
...
function buildProductStock($pdo, $category, $quantityInput, $sizesInput, &$error) {
    // Standard products use one quantity; clothing products calculate quantity from size-level stock
...
// New listings always start as pending so admins can review them before marketplace display
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'edit_product') {
    ...
    try {
        ...
        // Verify ownership before accepting edits, then reset approval fields for re-moderation
        $title = trim($_POST['title']);
        $description = trim($_POST['description']);
        $price = $_POST['price'];
        $category = $_POST['category'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_product') {
    ...
    try {
        ...
        // Products with orders are soft-deleted to preserve transaction history and receipts.
        ...
        if ($hasOrders) {
            ...
            // Remove unused uploaded images only after the product row is safely deleted
...
// Show only active seller-owned records; removed products stay hidden from the listings page
