...
// Only users with the admin role can access moderation, user, and transaction controls
$stmt = $pdo->prepare("SELECT role FROM users WHERE user_id=?");
$stmt->execute([$_SESSION['user_id']]);
$role = $stmt->fetchColumn();
if ($role !== 'admin') { echo "Access denied."; exit; }

// Release escrow payments that have passed their scheduled release time before loading dashboard data
releaseDueEscrow($pdo);

$success = '';

// Toggle account access without deleting the user's history or marketplace records
...
if (isset($_POST['update_transaction'])) {
    // Preserve the first manual release timestamp so escrow reporting stays auditable
...
// Approved products become visible only when stock is still available
...
if (isset($_POST['reject_product'])) {
    // Require a rejection reason so sellers know what to fix before resubmitting
...
// Recent transaction list includes both parties so admins can resolve buyer-seller disputes quickly
...
if (isset($_GET['view_product'])) {
    // Load a full product preview for moderation without leaving the dashboard context
