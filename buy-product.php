...
// Load the selected product with its category so availability and size rules can be checked
...
// Stop checkout for hidden, sold out, or unavailable products before showing payment fields
if ($product['status'] !== 'active' || (int)$product['quantity'] <= 0) {
    echo "This product is not available for purchase.";
    exit;
}

// Clothing products track stock per size as well as the total product quantity
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        // Lock the product row during checkout so two buyers cannot purchase the same final stock
        ...
        $sizeStock = $isClothing ? decodeClothingSizeStock($product['clothing_size_stock'] ?? '') : [];
        if ($isClothing) {
            // Validate the selected size and reduce only that size's stock count
        ...
        // Create a pending escrow transaction; the processing page handles the simulated payment step
        ...
        // Keep marketplace visibility in sync with remaining stock after the purchase
        if ($isClothing) {
    ...
    // Pass only the card's last four digits to the next step for display/reference
